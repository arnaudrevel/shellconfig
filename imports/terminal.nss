menu(type='*' where=(sel.count or wnd.is_taskbar or wnd.is_edit) title=title.terminal sep=sep.top image=icon.run_with_powershell)
{
	...
	item(title="Linux" cmd-ps=`-command wsl` image sep)
	item(title="AIBOOSTER" cmd-ps=`-command "ssh arnaud@192.168.10.103"` image)
	item(title="formationIA" cmd-ps=`-command "ssh ubuntu@54.37.51.188"` image)
	item(title="serveurarnaud" cmd-ps=`-command "ssh ubuntu@135.125.201.134"` image)
}
